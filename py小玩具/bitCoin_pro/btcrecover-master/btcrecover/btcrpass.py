...
import time, timeit, hashlib, collections, base64, struct, atexit, zlib, math, json, numbers
...
simple_typo_args["closecase"] = dict(action="store_true",
                                     help="like --typos-case, but only change letters next to those with a different case")
...
def prompt_unicode_password(prompt, error_msg):
    ...
    if 'utf' not in encoding.lower():
        print(prog + ": warning: terminal does not support UTF; passwords with non-ASCII chars might not work",
              file=sys.stderr)
        ...
        for i in xrange(10):
            try:
                ...
                if "not a valid Win32 application" in unicode(e):
                    print(prog + ": error: can't load Armory, 32/64 bit mismatch between it and Python",
                          file=sys.stderr)
                raise
            else:
...
class WalletArmory(object):
    ...
    def difficulty_info(self):
        return "{:g} MiB, {} iterations + ECC".format(round(self._kdf.getMemoryReqtBytes() / 1024 ** 2, 2),
                                                      self._kdf.getNumIterations())
    ...
    def init_opencl_kernel(self, devices, global_ws, local_ws, int_rate, save_every=1, calc_memory=False):
        ...
        assert len(devices) == len(global_ws) == len(
            local_ws), "WalletArmory.init_opencl_kernel: one global_ws and one local_ws specified for each device"
        assert save_every > 0
        ...
        self._cl_kernel_fill.set_scalar_arg_dtypes(
            [None, None, None, None, numpy.uint32, numpy.uint32, None, numpy.uint8])
        ...
        for i, device in enumerate(devices):
            ...
            max_local_ws = min(
                self._cl_kernel_fill.get_work_group_info(pyopencl.kernel_work_group_info.WORK_GROUP_SIZE, device),
            ...
            if local_ws[i] > max_local_ws:
                error_exit("--local-ws of", local_ws[i], "exceeds max of", max_local_ws,
                           "for GPU '" + device.name.strip() + "' with Armory wallets")
        ...
        if calc_memory:
            ...
            for i, device in enumerate(devices):
                ...
                print(
                    "  global memory size:     {:,} MiB".format(int(round(device.global_mem_size / float(1024 ** 2)))))
                print("  with --mem-factor {},".format(save_every if save_every > 1 else "1 (the default)"))
                ...
                print("      est. memory usage:  {:,} MiB\n".format(
                    int(round(global_ws[i] * mem_per_worker / float(1024 ** 2)))))
            sys.exit(0)
    ...
    def _return_verified_password_or_false_opencl(self, passwords):
        assert len(passwords) <= sum(
            self._cl_global_ws), "WalletArmory.return_verified_password_or_false_opencl: at most --global-ws passwords"
        ...
        for i in xrange(self._kdf.getNumIterations()):
            ...
            for v_start in xrange(0, v_len - self._v_len_chunksize, self._v_len_chunksize):
                ...
                for devnum in xrange(len(self._cl_devices)):
                        ...
                        self._cl_V_buffer0s[devnum], self._cl_V_buffer1s[devnum], self._cl_V_buffer2s[devnum],
                        self._cl_V_buffer3s[devnum],
            ...
            for devnum in xrange(len(self._cl_devices)):
                    ...
                    self._cl_V_buffer0s[devnum], self._cl_V_buffer1s[devnum], self._cl_V_buffer2s[devnum],
                    self._cl_V_buffer3s[devnum],
                    v_start + self._v_len_chunksize, v_len - self._v_len_chunksize - v_start,
                    self._cl_hashes_buffers[devnum], v_start < 0 and i == 0))
                self._cl_queues[devnum].flush()  # Starts the kernel
            pyopencl.wait_for_events(done)
...
class WalletBitcoinCore(object):
    ...
    def load_from_filename(cls, wallet_filename, force_purepython=False):
        ...
        if not mkey:
            if force_purepython:
                print(
                    prog + ": warning: bsddb (Berkeley DB) module not found; try installing it to resolve key-not-found errors (see INSTALL.md)",
                    file=sys.stderr)
    ...
    def init_opencl_kernel(self, devices, global_ws, local_ws, int_rate):
        ...
        assert hasattr(self,
                       "_iter_count") and self._iter_count, "WalletBitcoinCore.init_opencl_kernel: bitcoin core wallet or mkey has been loaded"
    ...
    def _return_verified_password_or_false_opencl(self, passwords):
        assert len(passwords) <= sum(
            self._cl_global_ws), "WalletBitcoinCore.return_verified_password_or_false_opencl: at most --global-ws passwords"
...
class WalletMultiBit(object):
    ...
    def is_wallet_file(wallet_file):
        wallet_file.seek(0)
        try:
            data = base64.b64decode(wallet_file.read(20).lstrip()[:12])
        except TypeError:
            return False
    ...
    def load_from_data_extract(cls, privkey_data):
        ...
        print(prog + ": WARNING: read the Usage for MultiBit Classic section of Extract_Scripts.md before proceeding",
              file=sys.stderr)
        self = cls(loading=True)
...
class WalletBitcoinj(object):
    ...
    def __init__(self, loading=False):
        ...
        if not pylibscrypt._done:
            print(prog + ": warning: can't find an scrypt library, performance will be severely degraded",
                  file=sys.stderr)
    ...
    def _load_from_filedata(cls, filedata):
        ...
        for key in pb_wallet.key:
            if key.type in (wallet_pb2.Key.ENCRYPTED_SCRYPT_AES, wallet_pb2.Key.DETERMINISTIC_KEY) and key.HasField(
                    "encrypted_data"):
                ...
                if encrypted_len == 48:
                    ...
                    return self
                print(prog + ": warning: ignoring encrypted key of unexpected length (" + unicode(encrypted_len) + ")",
                      file=sys.stderr)
...
class WalletMultiBitHD(WalletBitcoinj):
    ...
    def load_from_filename(cls, wallet_filename):
        ...
        if entropy_bits < 7.8:
            raise ValueError(
                "Doesn't look random enough to be an encrypted MultiBit HD wallet (only {:.1f} bits of entropy per byte)".format(
                    entropy_bits))
...
class WalletElectrum1(WalletElectrum):
    ...
    def _load_from_dict(cls, wallet):
        ...
        if seed_version is None:             raise ValueError(
            "Unrecognized wallet format (Electrum1 seed_version not found)")
        if seed_version != 4:                raise NotImplementedError(
            "Unsupported Electrum1 seed version " + unicode(seed_version))
...
class WalletElectrum2(WalletElectrum):
    ...
    def load_from_filename(cls, wallet_filename):
        ...
        if wallet.get("seed_version") not in (
        11, 12, 13) and wallet_type != "imported":  # all 2.x versions as of Oct 2016
    ...
    def return_verified_password_or_false(self, passwords):
        ...
        for count, password in enumerate(passwords, 1):
            ...
            if self._aes_library_name != 'aespython':
                ...
                plaintext_block = aes256_cbc_decrypt(keys[16:32], keys[:16],
                                                     self._ciphertext_beg)  # key, iv, ciphertext
                ...
                plaintext_block = aes256_cbc_decrypt(keys[16:32], self._ciphertext_end[:16],
                                                     self._ciphertext_end[16:])  # key, iv, ciphertext
    ...
    def load_from_filename(cls, wallet_filename, password=None, force_purepython=False):
        ...
        try:
            ...
            def decrypt_old():
                ...
                return decrypted[:-padding] if 1 <= padding <= 16 and decrypted[-padding] == b"\x80" and re.match(
                    b'{\s*"guid"', decrypted) else None
    ...
    if not force_purepython:
        try:
            ...
            if warnings and not missing_pbkdf2_warned:
                print(prog + ": warning: hashlib.pbkdf2_hmac requires Python 2.7.8+, using passlib instead",
                      file=sys.stderr)
        ...
        if sys.maxunicode < 65536:  # 2**16
            for c in s:
                ...
                if 0xD800 <= c <= 0xDBFF or 0xDC00 <= c <= 0xDFFF:
                    error_exit(error_msg,
                               "has character with code point > max (" + unicode(sys.maxunicode) + " / Unicode BMP)")
...
def expand_single_range(m):
    ...
    if char_first > char_last:
        raise ValueError(
            "first character in wildcard range '" + unichr(char_first) + "' > last '" + unichr(char_last) + "'")
...
def count_valid_wildcards(str_with_wildcards, permit_contracting_wildcards=False):
    ...
    for wildcard_set in re.findall(br"%[\d,i]*\[(.+?)\]|%%", str_with_wildcards):
        if wildcard_set:
            try:
                re.sub(br"(.)-(.)", expand_single_range, wildcard_set)
            except ValueError as e:
                return tstr(e)
    return count
...
def load_savestate(autosave_file):
    ...
    try:
        ...
        assert autosave_file.tell() <= SAVESLOT_SIZE, "load_savestate: slot 0 data <= " + unicode(
    ...
    if autosave_len > SAVESLOT_SIZE:  # if the second save slot is present
        ...
        try:
            ...
            assert autosave_file.tell() <= 2 * SAVESLOT_SIZE, "load_savestate: slot 1 data <= " + unicode(
                SAVESLOT_SIZE) + " bytes long"
    else:
...
def enable_pause():
    ...
    if pause_registered is None:
        if sys.stdin.isatty():
            ...
            print(prog + ": warning: ignoring --pause since stdin is not interactive (or was redirected)",
                  file=sys.stderr)
...
def init_parser_common():
    ...
    if not parser_common_initialized:
        ...
        parser_common.add_argument("--typos", type=int, metavar="COUNT",
                                   help="simulate up to this many typos; you must choose one or more typo types from the list below")
        ...
        for typo_name in itertools.chain(("swap",), simple_typo_args.keys(), ("insert",)):
            typo_types_group.add_argument("--max-typos-" + typo_name, type=int, default=sys.maxint, metavar="#",
        ...
        typo_types_group.add_argument("--max-adjacent-inserts", type=int, default=1, metavar="#",
                                      help="max # of --typos-insert strings that can be inserted between a single pair of characters (default: %(default)s)")
        ...
        parser_common.add_argument("--delimiter", metavar="STRING",
                                   help="the delimiter between tokens in the tokenlist or columns in the typos-map (default: whitespace)")
        ...
        parser_common.add_argument("--threads", type=int, default=cpus, metavar="COUNT",
                                   help="number of worker threads (default: number of CPUs, %(default)s)")
        parser_common.add_argument("--worker", metavar="ID#/TOTAL#",
                                   help="divide the workload between TOTAL# servers, where each has a different ID# between 1 and TOTAL#")
        parser_common.add_argument("--max-eta", type=int, default=168, metavar="HOURS",
                                   help="max estimated runtime before refusing to even start (default: %(default)s hours, i.e. 1 week)")
        ...
        parser_common.add_argument("--no-dupchecks", "-d", action="count", default=0,
                                   help="disable duplicate guess checking to save memory; specify up to four times for additional effect")
        ...
        parser_common.add_argument("--android-pin", action="store_true",
                                   help="search for the spending pin instead of the backup password in a Bitcoin Wallet for Android/BlackBerry")
        parser_common.add_argument("--blockchain-secondpass", action="store_true",
                                   help="search for the second password instead of the main password in a Blockchain wallet")
        ...
        parser_common.add_argument("--data-extract", action="store_true",
                                   help="prompt for data extracted by one of the extract-* scripts instead of using a wallet file")
        ...
        bip39_group.add_argument("--bip39", action="store_true",
                                 help="search for a BIP-39 password instead of from a wallet")
        bip39_group.add_argument("--mpk", metavar="XPUB", help="the master public key")
        bip39_group.add_argument("--addrs", metavar="ADDRESS", nargs="+",
                                 help="if not using an mpk, address(es) in the wallet")
        bip39_group.add_argument("--addressdb", metavar="FILE", nargs="?",
        ...
        bip39_group.add_argument("--bip32-path", metavar="PATH",
                                 help="path (e.g. m/0'/0/) excluding the final index (default: BIP-44 account 0)")
        ...
        bip39_group.add_argument("--wallet-type", metavar="TYPE",
        ...
        gpu_group.add_argument("--enable-gpu", action="store_true",
                               help="enable experimental OpenCL-based GPU acceleration (only supports Bitcoin Core wallets and extracts)")
        ...
        gpu_group.add_argument("--local-ws", type=int, nargs="+", default=[None], metavar="PASSWORD-COUNT",
                               help="OpenCL local work size; --global-ws must be evenly divisible by --local-ws (default: auto)")
        gpu_group.add_argument("--mem-factor", type=int, default=1, metavar="FACTOR",
                               help="enable memory-saving space-time tradeoff for Armory")
        gpu_group.add_argument("--calc-memory", action="store_true",
                               help="list the memory requirements for an Armory wallet")
        ...
        gpu_group.add_argument("--int-rate", type=int, default=200, metavar="RATE",
                               help="interrupt rate: raise to improve PC's responsiveness at the expense of search performance (default: %(default)s)")
...
def register_simple_typo(name, help=None):
    ...
    typo_types_group.add_argument("--max-typos-" + name, type=int, default=sys.maxint, metavar="#",
...
def parse_arguments(effective_argv, wallet=None, base_iterator=None,
    ...
    parser.add_argument("--max-tokens", type=int, default=sys.maxint, metavar="COUNT",
    ...
    parser.add_argument("--restore", metavar="FILE",
                        help="restore progress and options from an autosave file (must be the only option on the command line)")
    parser.add_argument("--passwordlist", metavar="FILE", nargs="?", const="-",
                        help="instead of using a tokenlist, read complete passwords (exactly one per line) from this file or from stdin")
    parser.add_argument("--has-wildcards", action="store_true",
                        help="parse and expand wildcards inside passwordlists (default: wildcards are only parsed inside tokenlists)")
    #
    ...
    if args.passwordlist or base_iterator:
        ...
        parser.add_argument("--passwordlist", required=not base_iterator, nargs="?", const="-", metavar="FILE",
                            help="instead of using a tokenlist, read complete passwords (exactly one per line) from this file or from stdin")
        parser.add_argument("--has-wildcards", action="store_true",
                            help="parse and expand wildcards inside passwordlists (default: disabled for passwordlists)")
        parser._add_container_actions(parser_common)
    ...
    if autosave_file:
        ...
        if tokenlist_file and tokenlist_file.peek() == b"#":  # if it's either a comment or additional args
            ...
            if re.match(b"#\s*--", first_line, re.UNICODE):  # if it's additional args, not just a comment
                print(
                    prog + b": warning: all options loaded from restore file; ignoring options in tokenlist file '" + tokenlist_file.name + b"'",
                    file=sys.stderr)
    ...
    for typo_name in itertools.chain(("swap",), simple_typos.keys(), ("insert",)):
        ...
        if typo_max < sys.maxint:
            ...
            if not args.__dict__["typos_" + typo_name]:
                print(prog + ": warning: --max-typos-" + typo_name + " is ignored without --typos-" + typo_name,
                      file=sys.stderr)
            #
            # Sanity check for a a --max-typos-* <= 0
            elif typo_max <= 0:
                print(prog + ": warning: --max-typos-" + typo_name, typo_max, "disables --typos-" + typo_name,
                      file=sys.stderr)
            ...
            # Sanity check --max-typos-* vs the total number of --typos
                ...
                print(prog + ": warning: --max-typos-" + typo_name + " (" + unicode(
                    typo_max) + ") is limited by the number of --typos (" + unicode(args.typos) + ")", file=sys.stderr)
    ...
    #
        ...
        if args.typos is None:
            if args.min_typos:
                print(prog + ": warning: --typos COUNT not specified; assuming same as --min_typos (" + unicode(
                    args.min_typos) + ")", file=sys.stderr)
    ...
    if args.max_adjacent_inserts != 1:
        if not args.typos_insert:
            print(prog + ": warning: --max-adjacent-inserts has no effect unless --typos-insert is used",
                  file=sys.stderr)
        elif args.max_adjacent_inserts < 1:
            print(prog + ": warning: --max-adjacent-inserts", args.max_adjacent_inserts, " disables --typos-insert",
                  file=sys.stderr)
            ...
            if args.max_typos_insert < args.typos:
                print(prog + ": warning: --max-adjacent-inserts (" + unicode(
                    args.max_adjacent_inserts) + ") is limited by --max-typos-insert (" + unicode(
                    args.max_typos_insert) + ")", file=sys.stderr)
            else:
                print(prog + ": warning: --max-adjacent-inserts (" + unicode(
                    args.max_adjacent_inserts) + ") is limited by the number of --typos (" + unicode(args.typos) + ")",
                      file=sys.stderr)
    ...
    if args.data_extract:
        ...
        if not key_crc_base64:
            if tokenlist_file == sys.stdin:
                print(
                    prog + ": warning: order of data on stdin is: optional extra command-line arguments, key data, rest of tokenlist",
                    file=sys.stderr)
        ...
        if isinstance(loaded_wallet, WalletArmory):
            print("WARNING: an Armory private key, once decrypted, provides access to that key's Bitcoin",
                  file=sys.stderr)
        #
        if isinstance(loaded_wallet, WalletMsigna):
            if args.msigna_keychain:
                print(
                    prog + ": warning: ignoring --msigna-keychain (the extract script has already chosen the keychain)")
    ...
    if args.enable_gpu or args.calc_memory:
        ...
        if args.local_ws[0] is not None:  # if one is specified, they're all specified
            for i in xrange(len(args.local_ws)):
                ...
                if args.global_ws[i] % args.local_ws[i] != 0:
                    error_exit("each --global-ws (" + unicode(
                        args.global_ws[i]) + ") must be evenly divisible by its --local-ws (" + unicode(
                        args.local_ws[i]) + ")")
                if args.local_ws[i] % 32 != 0 and not local_ws_warning:
                    print(prog + ": warning: each --local-ws should probably be divisible by 32 for good performance",
                          file=sys.stderr)
        ...
        for ws in args.global_ws:
            ...
            if ws % 32 != 0:
                print(prog + ": warning: each --global-ws should probably be divisible by 32 for good performance",
                      file=sys.stderr)
                break
        #
        extra_opencl_args = ()
    ...
    # if not --enable-gpu: sanity checks
        ...
        for argkey in "gpu_names", "global_ws", "local_ws", "int_rate", "mem_factor":
            if args.__dict__[argkey] != parser.get_default(argkey):
                print(prog + ": warning: --" + argkey.replace("_", "-"), "is ignored without --enable-gpu",
                      file=sys.stderr)
    ...
    if passwordlist_file:
        ...
        if passwordlist_file == sys.stdin:
            ...
            if not passwordlist_allcached and not args.no_eta:
                ...
                print(prog + ": warning: --no-eta has been enabled because --passwordlist is stdin and is large",
                      file=sys.stderr)
                args.no_eta = True
        #
    ...
    if args.no_eta:  # always true for --listpass and --performance
        if not args.no_dupchecks:
            if args.performance:
                print(
                    prog + ": warning: --performance without --no-dupchecks will eventually cause an out-of-memory error",
                    file=sys.stderr)
            elif not args.listpass:
                print(
                    prog + ": warning: --no-eta without --no-dupchecks can cause out-of-memory failures while searching",
                    file=sys.stderr)
...
def parse_mapfile(map_file, running_hash=None, feature_name=b"map", same_permitted=False):
    ...
    try:
        for line_num, line in enumerate(map_file, 1):
            ...
            if len(split_line) == 1:
                error_exit(feature_name, b"file '" + map_file.name + b"' has an empty replacement list on line",
                           line_num)
            if args.delimiter is None: split_line[1] = split_line[1].rstrip()  # ignore trailing whitespace by default

            check_chars_range(tstr().join(split_line), feature_name + b" file" + (
                b" '" + map_file.name + b"'" if hasattr(map_file, "name") else b""))
...
class AnchoredToken(object):
    ...
    def __init__(self, token, line_num="?"):
        if token.startswith(b"^"):
            ...
            # Else it's a begin anchor
                ...
                if len(token) > 1 and token[1] in b"0123456789,":
                    print(prog + ": warning: token on line", line_num,
                          "looks like it might be a positional or middle anchor, " +
        ...
        if self.text == "":
            print(prog + ": warning: token on line", line_num, "contains only an anchor (and zero password characters)",
                  file=sys.stderr)
    ...
    def __unicode__(self):
        return unicode(self.cached_str)

    # For hashlib
    def __repr__(self):
        return self.__class__.__name__ + b"(" + repr(self.cached_str) + b")"
    ...
    for line_num, line in enumerate(tokenlist_file, first_line_num):
        ...
        if line.startswith(b"#"):
            if re.match(b"#\s*--", line, re.UNICODE):
                print(prog + ": warning: all options must be on the first line, ignoring options on line",
                      unicode(line_num), file=sys.stderr)
            continue
    ...
    for map_filename in re.findall(br"%[\d,]*;(.+?);\d*b|%%", token):
        if map_filename and map_filename not in backreference_maps:
            ...
            backreference_maps[map_filename] = \
                parse_mapfile(open(map_filename, "r"), backreference_maps_sha1, b"backreference map",
                              same_permitted=True)
...
def expand_wildcards_generator(password_with_wildcards, prior_prefix=None):
    ...
    # Otherwise it's a contracting wildcard
        ...
        for remove_total in l_xrange(wildcard_minlen, l_min(wildcard_maxlen, max_from_left + max_from_right) + 1):
            ...
            for remove_right in l_xrange(l_max(0, remove_total - max_from_left),
...
def handle_oom():
    ...
    if password_dups and password_dups._run_number == 0:
        ...
        print(prog + ": notice: the --no-dupchecks option will reduce memory usage at the possible expense of speed",
              file=sys.stderr)
        return True
        ...
        print()  # move to the next line
        print(prog + ": error: out of memory", file=sys.stderr)
        print(prog + ": notice: the --no-dupchecks option can be specified twice to further reduce memory usage",
              file=sys.stderr)
        return True
    return False
...
def do_autosave(skip, inside_interrupt_handler=False):
    ...
    assert autosave_file.tell() <= start_pos + SAVESLOT_SIZE, "do_autosave: data <= " + unicode(
        SAVESLOT_SIZE) + " bytes long"
    autosave_file.flush()
    try:
        os.fsync(autosave_file.fileno())
            ...
            if is_displayed:
                ...
                if est_secs_per_password:
                    ...
                    if unskipped_passwords_counted > 0:
                        ...
                        if eta < 90:
                            eta = unicode(int(eta) + 1) + " minutes"  # round up
                        else:
                            eta /= 60
                            if eta < 48:
                                eta = unicode(int(round(eta))) + " hours"
                            else:
                                eta = unicode(round(eta / 24, 1)) + " days"
    ...
    if args.listpass:
        if tstr == unicode:
            ...
            if not stdout_encoding:
                ...
                print(prog + ": warning: stdout's encoding is not Unicode compatible; data loss may occur",
                      file=sys.stderr)
        else:
        ...
        try:
            for password in password_iterator:
                passwords_count += 1
                builtin_print(
                    password[0] if stdout_encoding is None else password[0].encode(stdout_encoding, "replace"))
        except BaseException as e:
        ...
        if l_savestate and b"total_passwords" in l_savestate and args.no_dupchecks:
            ...
            if l_savestate:
                if b"total_passwords" in l_savestate:
                    assert l_savestate[
                               b"total_passwords"] == passwords_count, "main: saved password count matches actual count"
                else:
