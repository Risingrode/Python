...
def create_valid_mnemonics(strength=128):
    ...
    b = (bin(int.from_bytes(rbytes, byteorder="big"))[2:].zfill(len(rbytes) * 8) + bin(int(h, 16))[2:].zfill(256)[
                                                                                   : len(rbytes) * 8 // 32])
...
def generate_mnem_address_pairs(counter, match, queue, r):
    ...
    while True:
        ...
        if k % screen_print_after_keys == 0:
            ...
            print('[+] Total Keys Checked : {0}  [ Speed : {1:.2f} Keys/s ]  Current words: {2}'.format(counter.value,
                                                                                                        counter.value / (
                                                                                                                time.time() - st),
                                                                                                        mnem))
