...
def pick_words(entropy, entropy_hash, checksum_bit_count):
    ...
    bit = entropy_hash[
          0:checksum_char_count]  # *take first x bit of bits (x is not defined but be added to slice manually)
    ...
    groups = [source[i:i + 11] for i in
              range(0, len(source), 11)]  # splits the total bits into groups of 11 bits at a time
    ...
    indices = [int(str('0b') + source[i:i + 11], 2) for i in
               range(0, len(source), 11)]  # (str('0b') for i in range(0,len(source),11))
