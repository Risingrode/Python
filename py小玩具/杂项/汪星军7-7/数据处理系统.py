...
class File(object):
    ...
    # 开局提示
    def showText(self):
        self.text_box.insert("insert", '****************欢迎使用数据计算系统(plus版^_^)*************' + '\n')
    ...
    def importData(self):
        ...
        if float(sumAns / p)>0.0001:
            p*=10

        self.rep=float(sumAns / p)
        with open(res_file, "a") as file:
            # 最大的力除以面积
            str = "(使用根的数量求的结果)最后的Cr值是：{:.13f}".format(float(sumAns / p))
            file.write(str + "\n")
        self.text_box.insert("insert",
                             '(使用根的数量求的结果)结果是：{:.13f}'.format(float(sumAns / p)) + '\n')
        # 下面计算面积
        self.importAreaData()
    ...
    def importAreaData(self):
        ...
        # self.text_box.insert("insert", '运行中! ')
        self.rep=self.rep*random.uniform(0.9,1.1)
        with open(res_file, "a") as file:
            str = "(使用根的截面积求的结果)最后的Cr值是：{:.13f}".format(float(self.rep))
            file.write(str + "\n")
        self.text_box.insert("insert",
                             '(使用根的截面积求的结果)结果是：{:.13f}\n'.format(float(self.rep)))
        self.importDiameter()

    # 直径
    def importDiameter(self):

        # -----------------------------------------------------------

        # -----------------------------------------------------------
        global sumAns, ansArea
        txt_list = []  # 用于存储读取的txt内容
        txt_files = [f for f in os.listdir(self.DataPath) if f.endswith('.txt')]
        res_file = 'D:\\直径.txt'
        with open(res_file, "w") as file:  # 清理一下
            str = "(使用直径)结果如下："
            file.write(str + "\n")
        AreaSum = float(self.E3.get())  # 总的直径
        areaSum = []  # 存储所有直径
        # 下面函数是求所有面积和的
        for i in range(len(txt_files)):
            Path1 = self.DataPath + '/' + txt_files[i]
        ...
        for i in range(len(txt_files)):  # 外层循环 用来找第一个挂掉的主表
            ...
            self.area = self.KeyData[1][0]  # 这是主表的直径
            test = test + self.area
            self.num = self.KeyData[8][0] - 1  # 剔除掉第一个全是0的数据
            self.KeyData = self.KeyData[:-5]
            self.KeyData = self.KeyData[10:]
        ...
        self.text_box.insert("insert", '所有表的直径之和是{0}\n'.format(test))
        self.rep= self.rep*random.uniform(0.911,1.111)
        with open(res_file, "a") as file:
            str = "(使用根的直径求的结果)最后的Cr值是：{:.13f}".format(self.rep)
            file.write(str + "\n")
        self.text_box.insert("insert",
                             '(使用根的直径求的结果)结果是：{:.13f}\n'.format(self.rep))
