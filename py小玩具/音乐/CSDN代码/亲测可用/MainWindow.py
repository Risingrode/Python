...
class KgyySpider(object):
    def __init__(self):
        ...
        menuFile.add_command(label='新窗体', command=self.Window2)
        ...
        self.List_url = []
        # 12.选择输入框
        self.input_num = tk.Entry(self.window, show=None, font=('Kaiti', 12), textvariable=self.prompt_words_1)
        self.input_num.grid(row=6, column=1, sticky=W + E + N + S, padx=10)
        # 13.下载按钮
        self.download_button = tk.Button(self.window, text='下载', font=('Kaiti', 12), width=10, height=1,
                                         command=self.Download)
        self.download_button.grid(row=9, column=1, pady=10, sticky=W + N + S, padx=10)
        # 14.下载说明
        self.prompt_words_3 = tk.Label(self.window, text='输入音乐序号进行下载', font=('Kaiti', 12), width=10,
                                       height=2, )
        self.prompt_words_3.grid(row=8, column=1, pady=1, sticky=W + E + N + S, padx=10)
        # 15.一键下载
        self.download_button = tk.Button(self.window, text='下载全部', font=('Kaiti', 12), width=10, height=1,
                                         command=self.DownloadAll)
        self.download_button.grid(row=9, column=1, pady=10, sticky=E + N + S, padx=10)
        # 16.弹出框
        self.lb = tk.Label(self.window, text='')
        # 17.选择下载地址
        self.DownPath = ''
        self.download_path = tk.Button(self.window, text='下载地址选择', font=('Kaiti', 12), width=10, height=1,
                                       command=self.DownloadPath)
    ...
    def DownloadAll(self):  # 下载全部
        for i in range(len(self.List_name)):
            self.requests_song_url(self.List_name[i], self.List_url[i])
        self.PopWin('一键下载成功！')
    ...
    def DownloadPath(self):
        self.DownPath = tkinter.filedialog.askdirectory()
    ...
    def save_data(self, song_name, song_content):
        ...
        try:
            if (self.DownPath):
                with open(self.DownPath + '/{}.mp3'.format(song_name), 'wb') as f:
